...
public class IndicatorOfLauncher extends AbstractHudElement {
	...
	private final static Color BACKGROUND_COLOR_DISABLE= new  Color(19d / 255d, 73d / 255d, 95d / 255d, 0.8d);
	
	private final static Color BACKGROUND_BAR_COLOR = new Color(30d / 255d, 127d / 255d, 169d / 255d, 1d);
	private final static Color BACKGROUND_BAR_COLOR_DISABLE= new  Color(19d / 255d, 73d / 255d, 95d / 255d, 1d);
	...
	public IndicatorOfLauncher(MatchManager matchManager) {
		super(matchManager);

		bar = new Rectangle(BAR_WIDTH,BAR_HEIGHT,BACKGROUND_COLOR_DISABLE);
		shadowBar = new Rectangle(BAR_WIDTH,BAR_HEIGHT,BACKGROUND_COLOR_DISABLE);
//		shadowBar.setStroke(Color.BLACK);
		...
		coloredPowerBar = new ImageView(new Image("file:image/indicatorePotenza.png"));
		powerBar = new ImageView(new Image("file:image/barraPotenza.png"));
		arrow = new ImageView(new Image("file:image/freccia.png"));
		launcherIndicator = new ImageView(new Image("file:image/indicatoreLaucher.png"));
		...
		launcherIndicator.relocate(BAR_WIDTH / 2+1, -launcherIndicator.getFitHeight() / 2+1);
		background.relocate(BAR_WIDTH / 2, -launcherIndicator.getFitHeight() / 2);

		coloredPowerBar.relocate(-coloredPowerBar.getFitWidth() + 1, -coloredPowerBar.getFitHeight() / 2);
		powerBar.relocate(-coloredPowerBar.getFitWidth() + 1, -coloredPowerBar.getFitHeight() / 2);
		bar.relocate(-coloredPowerBar.getFitWidth() + 1, -coloredPowerBar.getFitHeight() / 2);
		shadowBar.relocate(-coloredPowerBar.getFitWidth() + 1, -coloredPowerBar.getFitHeight() / 2);
		
		arrow.relocate(-arrow.getFitHeight() / 2+BAR_WIDTH / 2, -arrow.getFitHeight() / 2);
		root.getChildren().addAll(shadowBar,background,coloredPowerBar,bar,powerBar, launcherIndicator, arrow);
	...
	public void draw() {
		...
		root.relocate(DISTANCE_SCREEN_LEFT, scene.getHeight() - DISTANCE_SCREEN_BOT);
		rotationArrow.setPivotY(arrow.getY() + arrow.getFitHeight() / 2);
		rotationArrow.setPivotX(arrow.getX() + arrow.getFitHeight() / 2);
		...
		if (matchManager.getCurrentPlayer().getLauncher().isActivated()) {
			...
			if (matchManager.getCurrentPlayer().getCurrentDirection() == Character.RIGHT) {
				rotationArrow.setAngle(-Math.toDegrees(matchManager.getCurrentPlayer().getLauncher().getAngle()));
			}
			else{
				rotationArrow.setAngle(Math.toDegrees(matchManager.getCurrentPlayer().getLauncher().getAngle()-45-90));
			}
		}else{
			background.setFill(BACKGROUND_COLOR_DISABLE);
		...
		if(keyPressed){
			...
			bar.setHeight(BAR_HEIGHT - BAR_HEIGHT*launcherPower/Launcher.MAX_LAUNCH_POWER);
		}else{
			coloredPowerBar.setVisible(false);
